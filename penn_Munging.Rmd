...
merged_AmphibianCaptureSurvey <- right_join(right_join(munged_Amphibian_Captured_Information,
  ...
  select(!c("...1.x.x", "fulcrum_id.x", "created_at.x.x","...1.y.x", "fulcrum_id.y.x", "created_at.y.x", "...1.x.y", 
            "fulcrum_id.y.y", "created_at.x.y", "...1.y.y", "fulcrum_id.y.y.y","created_at.y.y", "time_of_capture.y", 
...
cap_animal_table <- merged_AmphibianCaptureSurvey %>% 
  ...
  select(!c(visit_comment)) 

re_cap_table <- cap_animal_table %>% 
  filter(capture_type == "recapture")

cap_animal_table <- cap_animal_table%>% 
...
#### bind all animal table for uniquness
```{r}
